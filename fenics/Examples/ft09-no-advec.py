"""
Changed to take out the advections terms to see if it still works
8/27/22
...
F = ((u_1 - u_n1) / k)*v_1*dx  \
  + 1.0*eps*dot(grad(u_1), grad(v_1))*dx + K*u_1*u_2*v_1*dx  \
  + ((u_2 - u_n2) / k)*v_2*dx  \
  + 1.0*eps*dot(grad(u_2), grad(v_2))*dx + K*u_1*u_2*v_2*dx  \
  + ((u_3 - u_n3) / k)*v_3*dx  \
  + 0.10*eps*dot(grad(u_3), grad(v_3))*dx - K*u_1*u_2*v_3*dx + K*u_3*v_3*dx \
...
vtkfile_u_1 = File('no_advec_reaction_system/u_1.pvd')
vtkfile_u_2 = File('no_advec_reaction_system/u_2.pvd')
vtkfile_u_3 = File('no_advec_reaction_system/u_3.pvd')
